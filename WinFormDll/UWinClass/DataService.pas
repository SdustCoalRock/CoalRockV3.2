...
var
 sql1,sql2,pramtart,Gzmid:string;
 MySqlDataSet1:TMyDataSet;
begin
    pramtart:='';
     MySqlDataSet1:=TMyDataSet.Create(nil);
     MySqlDataSet1.MySqlConnection :=MainDataModule.ExConn;
//    sql:=' select cqid as AA from cqcsb as c,WorkFaceBuillDrill as w where c.kid='+inttostr(Mkid) + ' and c.qyzt=1' +
//         ' w.gzmid= c.cqid ';
     sql1:=' select cqid as AA from cqcsb as c  where c.kid='+inttostr(Mkid) ;
     ...
     try
         ...
         if MainDataModule.MyDataSet.Open  then
           while not MainDataModule.MyDataSet.Eof do begin
               Gzmid:=MainDataModule.MyDataSet.FieldByName('AA').AsString;
               sql2:=' select zkid from  WorkFaceBuillDrill where  gzmid= '+Gzmid;
               MySqlDataSet1.Close ;
               MySqlDataSet1.CommandText :=sql2;
               if MySqlDataSet1.Open  then
                  if MySqlDataSet1.RecordCount >0 then  begin
                     pramtart:=Gzmid+ ',' + MySqlDataSet1.FieldByName('zkid').AsString;
                     break;
                  end;
               MySqlDataSet1.Close ;
...
function TDataService.GzmAndZkIsEffective(Gzmid, zkid: String): Boolean;
var
  Sql:string;
  McId:Integer;
begin
    ...
    try
     ...
     if   MainDataModule.MyDataSet.Open then
        if   MainDataModule.MyDataSet.RecordCount >0 then
            MCid:=MainDataModule.MyDataSet.FieldByName('mcid').AsInteger ;
     MainDataModule.MyDataSet.Close();
