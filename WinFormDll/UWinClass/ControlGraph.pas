...
type
  TControlImage=class
    ...
    public
        ...
        procedure SetMyColorBox(ColorBox:TColorBox);
        function  GetOutStr:String;
        procedure InitDrawGraph;
        //setLine  Rect
        procedure SetDrawLine;   //画直线
        procedure SetDrawRectang; //画矩形
        procedure SetDrawEllips;  //画椭圆
        procedure SetDrawCircle;  //画圆
        procedure SetFillImage;  //填充区域
        procedure SetImageMove;  //移动图片
        procedure SetImageDelete; //删除图片
        procedure SetImageClear; //清除图片
...
procedure TControlImage.InitDrawGraph;
begin

  Myimage.Canvas.Brush.Style:=bsClear;
  Myimage.Canvas.Pen.Color :=Mycolorbox.Selected;
...
begin
  ...
  while itsLine<>nil do    begin
      if prepareLine(myPoint,itsLine^.originP,itsLine^.finalP) then   begin
        OutStr:='继续刚才的操作类型，或者重新选择操作类型';
  ...
  if ctrlRect then    begin
    dRect:=firstRect ;
    itsRect:=firstRect^.next ;
    while itsRect<>nil do   begin
      if prepareRect(myPoint,itsRect^.originP,itsRect^.finalP) then    begin
        OutStr:='继续刚才的操作类型，或者重新选择操作类型';
  ...
  if ctrlEllips then   begin
    dEllips:=firstEllips;
    itsEllips:=firstEllips^.next ;
    while itsEllips<>nil do     begin
      if prepareEllips(myPoint,itsEllips^.originP ,itsEllips^.finalP) then        begin
        OutStr :='继续刚才的操作类型，或者重新选择操作类型';
  ...
  if ctrlCircle then   begin
    dCircle:=firstCircle;
    itsCircle:=firstCircle^.next ;
    while itsCircle<>nil do     begin
      if prepareCircle(myPoint,itsCircle^.centreP ,itsCircle^.edgeP )then        begin
        OutStr :='继续刚才的操作类型，或者重新选择操作类型';
  ...
  begin
    ...
    while itsRect<>nil do   begin
      if prepareRect(myPoint,itsRect^.originP,itsRect^.finalP) then    begin
        ...
        MyImage.Canvas.Brush.Color :=itsRect^.fillColor;
        drawRect(itsRect^.originP ,itsRect^.finalP);
        itsRect^.fillColor :=Mycolorbox.Selected ;
        MyImage.Canvas.Brush.Color :=itsRect^.fillColor ;
        drawRect(itsRect^.originP ,itsRect^.finalP);
  ...
  if ctrlEllips then   begin
    ...
    while itsEllips<>nil do     begin
      if prepareEllips(myPoint,itsEllips^.originP ,itsEllips^.finalP) then        begin
        ...
        MyImage.Canvas.Brush.Color :=itsEllips^.fillColor ;
        drawEllips(itsEllips^.originP ,itsEllips^.finalP);
        itsEllips^.fillColor := MYcolorbox.Selected ;
        MyImage.Canvas.Brush.Color := itsEllips^.fillColor ;
        drawEllips(itsEllips^.originP ,itsEllips^.finalP);
  ...
  begin
    ...
    begin
      if prepareCircle(myPoint,itsCircle^.centreP ,itsCircle^.edgeP )then        begin
        ...
        itsCircle^.fillColor := MYcolorbox.Selected ;
        MyImage.Canvas.Brush.Color := itsCircle^.fillColor ;
        drawCircle(itsCircle^.centreP ,itsCircle^.edgeP );
        //image1.Canvas.FloodFill(itsCircle^.centreP.X ,itsCircle^.centreP.Y ,image1.Canvas.Brush.Color,fsBorder);
        itsCircle:=lastCircle;
...
procedure TControlImage.SetDrawCircle;
begin
  ...
  OutStr :='绘制圆：拾取圆心（按下鼠标左键）';
end;

procedure TControlImage.SetDrawEllips;
begin
...
begin
  ...
  moving:=false;
  deling:=false;
  OutStr :='绘制矩形：拾取起始点（按下鼠标左键）';

end;

procedure TControlImage.SetFillImage;
begin
  filling:=true;
  moving:=false;
  deling:=false;

  OutStr:='用所选颜色填充图形（请选择图形的轮廓线）';
end;
...
procedure TControlImage.SetImageClear;
begin
   OutStr :='清除所有图形';

  if MessageBox(Application.Handle ,'您确认删除所有图形？','清除确认',MB_YESNOCANCEL)=IDYES then    begin
     realClear;
  end;
  myDrawStart:=null;
  filling:=false;
...
procedure TControlImage.SetImageDelete;
begin
  deling:=true;
  filling:=false;
  moving:=false;

  OutStr:='删除图形（请选择图形的轮廓线）';
end;

procedure TControlImage.SetImageMove;
begin
  moving:=true;
  filling:=false;
  deling:=false;
  OutStr :='移动图形（请选择图形的轮廓线）';
end;

procedure TControlImage.SetMyColorBox(ColorBox: TColorBox);
begin
    MyColorBox:=ColorBox;
end;
