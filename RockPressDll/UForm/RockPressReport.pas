...
type
    ...
    private
       ...
       //
        ...
        procedure Add_Third_One_Main(P_Str:string;inGZm:TGzm);// 3.1 数据分析综述
        procedure Add_Third_One_First(P_Str:string;inGZm:TGzm);// 3.1.1  主要分析参数
        procedure Add_Third_One_Second(P_Str:string;inGZm:TGzm);// 3.1.2  技术参数统计分析表
        function  Add_Third_One_Second_Add_fulu_Bmp(PhaseNumber:Integer):string;// 给附录增加 图形
        procedure Add_Third_Second_Main(P_Str:string;inGZm:TGzm);// 3.2 主控程序
        procedure Add_Third_Two_First(P_Str:string;inGZm:TGzm);// 3.2.1  分析的主要数据
        Procedure ADd_Third_Two_First_Table(P_Str:string;Modify:integer);// 添加表格
        procedure Add_Third_Two_First_Table_Child(phaseNumber:integer;OneStepIn: TcomprehensiveOldStep;
           sup_st,sup_ed:integer;itab:Table; var come_i,iRow:integer);// 为添加表格服务
        procedure Add_Third_three_Main(P_Str:string;inGZm:TGzm);// 3.3  分阶段来压信息 通报
        procedure Add_Third_three_Main_Chlid(phaseNumber,P_c,modify:integer;P_Str:string;inGZm:TGzm);// 3.3  分区域来压信息 通报
        function  Add_Third_three_Main_Chlid_Add_fulu_Bmp(PhaseNumber:Integer):string;// 给附录增加 图形
...
begin
     ...
     with innword do begin
        ...
        //第四张图  把 平均值的预测 反应出来
         ...
         Form_Band_Bar.Close ;
         // 书签再回来
        //GotoBookMark('tempPosition');
        //删除该书签
        //DeleteBookMark('tempPosition');
     end;
...
begin
   ...
   DirName:=DLL_path+'\InputData\WorkDace_'+IntTostr(inGzm.Gzmbh)+'\';
    with innWord do begin
      ...
      str:='   通过图'+IntTostr(SencentId)+'-14与图'+IntTostr(SencentId)+'-15.可以非常清晰的看出【'+
      ...
      str:='   根据图'+IntTostr(SencentId)+'-3到图'+IntTostr(SencentId)+'-7描述的采煤工作面周边支承压力的转移规律，' +
            ...
            '】工作面的支承压力分布规律进行计算，得到结果如表'+IntTostr(SencentId)+'-1 所示：';
      SetFont('宋体',12,false,false);  AlignLine(0); WriteText(str,1);   AddReturn();
      Report_Zcyl(inGzm);
      //2.2.4
      SetFont('宋体',12,false,false);
...
begin
    with innWord do begin
        ...
        WriteText(istr,1); AddReturn();
         ...
         istr:='   第五：“寻找大显现、关注异常点”矿压显现分析要有一定的数据对照区间，最小进刀区间要选择50刀以上。';
        WriteText(istr,1); AddReturn();
    end;
end;
...
begin
   ...
   Str_Secents:=intTostr(SencentId);
    with innWord do begin
      ...
      //第三章 第二节
      Add_Third_Second_Main(Str_Secents,inGZm);
      //第三节 或者更多的节
      Add_Third_three_Main(Str_Secents,inGZm);
    end;
end;

procedure TWordReport.Add_Third_Second_Main(P_Str: string; inGZm: TGzm);
var
 istr:string;
begin
     ...
     with innWord do begin
          // 第二节
         SetFont('宋体',14,false,false);
         SetHeading(P_Str+'.2 '+'采煤工作面矿压显现整体特征分析',2);// 章节
         ...
         WriteText(istr,1);  AddReturn();
         // 第一小节
         Add_Third_Two_First(P_Str,inGZm);
...
procedure TWordReport.Add_Third_Two_First(P_Str: string; inGZm: TGzm);
var
  tab:Table;
...
begin
    ...
    with innWord do begin
         SetFont('宋体',12,false,false);
         SetHeading(P_Str+'.2.1 采煤工作面矿压显现显著整体特征分析',3);// 章节
         //
         ...
         SetHeading('表'+P_Str+'-'+intToStr(Third_Tab_id)+'【'+MainDataModule.WorkFace_Name+
                   '工作面】整体分析矿压显现显著数据表',4);
         ADd_Third_Two_First_Table(P_Str,10);
         ...
         //
         SetFont('宋体',12,false,false);
         SetHeading(P_Str+'.2.2 采煤工作面矿压显现明显整体特征分析',3);// 章节
         ...
         SetHeading('表'+P_Str+'-'+intToStr(Third_Tab_id)+'【'+MainDataModule.WorkFace_Name+
                   '工作面】整体分析矿压显现明显数据表',4);
         ADd_Third_Two_First_Table(P_Str,9);
         Addreturn();
          //
         SetFont('宋体',12,false,false);
         SetHeading(P_Str+'.2.3 采煤工作面局部矿压显现异常整体特征分析',3);// 章节
         //
         R_Str:='    本次数据分析中，共提取' +
             '【'+intTostr(iDwGrap.Auto_Judge_OldStep.GetComeStepCount(8)+iDwGrap.Auto_Judge_OldStep.GetComeStepCount(5)+
         ...
         SetHeading('表'+P_Str+'-'+intToStr(Third_Tab_id)+'【'+MainDataModule.WorkFace_Name+
                   '工作面】局部矿压显现异常数据表',4);
         ADd_Third_Two_First_Table(P_Str,-1);

         Addreturn();
    end;
end;

procedure TWordReport.ADd_Third_Two_First_Table(P_Str: string; Modify: integer);
var
           ...
           for I := 0 to length(iDwGrap.Auto_Judge_OldStep.ExSteps)-1 do begin
              ...
              if not((iDwGrap.Auto_Judge_OldStep.ExSteps[i].OneStepInfo.EndDao>iDwGrap.Auto_Judge_OldStep.ExSteps[i].OneStepInfo.StartDao ) and
                 (iDwGrap.Auto_Judge_OldStep.ExSteps[i].OneStepInfo.EndSup>iDwGrap.Auto_Judge_OldStep.ExSteps[i].OneStepInfo.StartSup )) then
                     continue;
             // 把数据写入表格
             Add_Third_Two_First_Table_Child(-1,iDwGrap.Auto_Judge_OldStep.ExSteps[i].OneStepInfo,
...
begin
    ...
    with innWord do begin
       ...
       WriteText('   备注:附录中图V-'+IntTostr(PinPutuid)+'集中展示了全工作面矿压数据频谱图，辅助显示了随机选取的12个支架的压力频谱图。',1);
       AddReturn();
...
begin
     ...
     with innWord do begin
         ...
         SetFont('宋体',12,false,false);
         SetHeading(P_Str+'.1.1 矿压监测数据整体技术参数分析',3);// 章节
         SetFont('宋体',12,False,False);
         ...
         SetFont('宋体',12,false,false);
         SetHeading(P_Str+'.1.2 矿压监测数据统计学特征分析',3);// 章节
         SetFont('宋体',12,False,False);
         istr:='    为便于数据统计，基于曲面的矿压分析方法是以每个支架的一次循环（进刀）为基准进行矿压分析，' +
               '提取每个支架、每次循环的初撑力、末阻力、最大值、平均值及某一时刻支架瞬间监测值等五类数据，' +
               '在分析各类数据的统计学特征基础上，对数据进行穿透、交叉融合，寻找矿压监测数据的内在联系。分析结果如表'+
               P_Str+'-2到表'+P_Str+'-'+intTostr(iDwGrap.Auto_Judge_OldStep.PhaseNumber+1) +'。';
         WriteText(istr,1); AddReturn();
         // 第三章 第一节  第二小节
         Add_Third_One_Second(P_Str,inGZm);
...
procedure TWordReport.Add_Third_One_Second(P_Str: string; inGZm: TGzm);
var
  i,k,Sub_id:integer;
  Tab:Table;
  Str_Out:string;
   ...
   {ChangeThreeArrayContourGrid}
    ...
    with innWord do begin
      for I := 0 to iDwGrap.Auto_Judge_OldStep.PhaseNumber-1 do  begin
         ...
         SetFont('宋体',11,true,false);AlignLine(1);
         inc(third_Tab_id);
         SetHeading('表'+P_Str+'-'+IntTostr(third_Tab_id)+' 第'+Public_Basic.GetChineseNumber(i+1)+'区间【'+
                    IntToStr(iDWGrap.Auto_Judge_OldStep.PhaseSteps[i].StartsupBh) +'-'+
                    IntToStr(iDWGrap.Auto_Judge_OldStep.PhaseSteps[i].EndSupBh) +'】号支架统计学特征分析',4);
         SetFont('宋体',10,false,false) ;
         AlignLine(0);
         Tab:=AddTable(6,18);
         ...
         SetFont('宋体',9,true,false) ; AlignLine(0); WriteText('区间编号',0); Moveright(1);
                SetFont('宋体',9,False,false) ; AlignLine(1); WriteText(InttoStr(i+1),0); Moveright(1);
         ...
         for k := 0 to 4 do   begin // 五种数据类型
             ...
             SetFont('宋体',9,false,false) ; AlignLine(1);
                WriteText(FormatFloat('0.00',iDwGrap.Auto_Judge_OldStep.PhaseSteps[i].Statistical_data.Get_AGV),0); MoveDown(1);   // 第4 行
             SetFont('宋体',9,false,false) ; AlignLine(1);
                WriteText(FormatFloat('0.00',iDwGrap.Auto_Judge_OldStep.PhaseSteps[i].Statistical_data.Get_Standard_erro),0); MoveDown(1);   // 第5 行
             SetFont('宋体',9,false,false) ; AlignLine(1);
                WriteText(FormatFloat('0.00',iDwGrap.Auto_Judge_OldStep.PhaseSteps[i].Statistical_data.Get_Median),0); MoveDown(1);   // 第6 行
             SetFont('宋体',9,false,false) ; AlignLine(1);
                WriteText(FormatFloat('0.00',iDwGrap.Auto_Judge_OldStep.PhaseSteps[i].Statistical_data.Get_Model),0); MoveDown(1);   // 第7 行
             SetFont('宋体',9,false,false) ; AlignLine(1);
                WriteText(FormatFloat('0.00',iDwGrap.Auto_Judge_OldStep.PhaseSteps[i].Statistical_data.Get_Standard_Deviation),0); MoveDown(1);   // 第8 行
             SetFont('宋体',9,false,false) ; AlignLine(1);
                WriteText(FormatFloat('0.00',iDwGrap.Auto_Judge_OldStep.PhaseSteps[i].Statistical_data.Get_Variance),0); MoveDown(1);   // 第9 行
             SetFont('宋体',9,false,false) ; AlignLine(1);
                WriteText(FormatFloat('0.00',iDwGrap.Auto_Judge_OldStep.PhaseSteps[i].Statistical_data.Get_peakedness),0); MoveDown(1);   // 第10 行
             SetFont('宋体',9,false,false) ; AlignLine(1);
                WriteText(FormatFloat('0.00',iDwGrap.Auto_Judge_OldStep.PhaseSteps[i].Statistical_data.Get_bs),0); MoveDown(1);   // 第11 行
             SetFont('宋体',9,false,false) ; AlignLine(1);
                WriteText(FormatFloat('0.00',iDwGrap.Auto_Judge_OldStep.PhaseSteps[i].Statistical_data.Get_max_Min),0); MoveDown(1);   // 第12 行
              SetFont('宋体',9,false,false) ; AlignLine(1);
                WriteText(FormatFloat('0.00',iDwGrap.Auto_Judge_OldStep.PhaseSteps[i].Statistical_data.Get_Min_Data),0); MoveDown(1);   // 第13 行
              SetFont('宋体',9,false,false) ; AlignLine(1);
                WriteText(FormatFloat('0.00',iDwGrap.Auto_Judge_OldStep.PhaseSteps[i].Statistical_data.Get_Max_Data),0); MoveDown(1);   // 第14 行
             SetFont('宋体',9,false,false) ; AlignLine(1);
                WriteText(FormatFloat('0.00',iDwGrap.Auto_Judge_OldStep.PhaseSteps[i].Statistical_data.Get_ToTal),0); MoveDown(1);   // 第15 行
             SetFont('宋体',9,false,false) ; AlignLine(1);
                WriteText(IntTostr(iDwGrap.Auto_Judge_OldStep.PhaseSteps[i].Statistical_data.Get_Count),0); MoveDown(1);   // 第16 行
             SetFont('宋体',9,false,false) ; AlignLine(1);
                WriteText(FormatFloat('0.00',iDwGrap.Auto_Judge_OldStep.PhaseSteps[i].Statistical_data.Get_ZXQD95),0);   // 第17 行

         end;
         // 第18行
          TableCellMerge(Tab,1,18,6,18); MoveDown(1);
          SetFont('宋体',9,False,false) ; AlignLine(0);
                WriteText(Add_Third_One_Second_Add_fulu_Bmp(i),0);
          // 跳出单元格
...
function TWordReport.Add_Third_One_Second_Add_fulu_Bmp(PhaseNumber:Integer):string;
{
    ...
    try
       //打开直方图模式
         ...
         if PathFile<>'' then  begin
            ...
            iStr:=iStr+'压力频谱图附录图 X-'+IntToStr(fuLu_Tu_id) +',';
         end;
         Form_Band_Bar.Close ;


     // 打开 曲线图 绘制界面
        CreateLineGraph(application.Handle,'',0,0,2);
        ...
        PathFile:=Form_LineGraph.SaveChartToBmp;
         if PathFile<>'' then  begin
            ...
            iStr:=iStr+'压力曲线图（六坐标轴）附录图 X-'+IntToStr(fuLu_Tu_id) +',';
         end;
       ...
       if PathFile<>'' then  begin
          ...
          iStr:=iStr+'压力曲线图1附录图 X-'+IntToStr(fuLu_Tu_id) +',';
       end;
       //
       ...
       if PathFile<>'' then  begin
          ...
          iStr:=iStr+'压力曲线图2附录图 X-'+IntToStr(fuLu_Tu_id) +',';
       end;
       ...
       if PathFile<>'' then  begin
          ...
          iStr:=iStr+'压力曲线图3附录图X-'+IntToStr(fuLu_Tu_id) +'。';
       end;
...
procedure TWordReport.Add_Third_three_Main(P_Str: string; inGZm: TGzm);
var
         ...
         for P := 0 to iDWGrap.Auto_Judge_OldStep.PhaseNumber -1 do  begin
             ...
             if ComeStep>0  then   begin
                 ...
                 SetHeading('表'+P_Str+'-'+intToStr(Third_Tab_id)+' 第【'+Public_Basic.GetChineseNumber(P+1)+
                           '】区间矿压显现显著区域信息表',4);
                 Add_Third_three_Main_Chlid(p,P_c,10,P_Str,inGZm);
             end;
            //----------
             ...
             if ComeStep>0  then   begin
                 ...
                 SetHeading('表'+P_Str+'-'+intToStr(Third_Tab_id)+' 第【'+Public_Basic.GetChineseNumber(P+1)+
                           '】区间矿压显现明显区域信息表',4);
                  Add_Third_three_Main_Chlid(p,P_c,9,P_Str,inGZm);
             end;
            //----------
             ...
             if ComeStep>0  then   begin
                 ...
                 SetHeading('表'+P_Str+'-'+intToStr(Third_Tab_id)+' 第【'+Public_Basic.GetChineseNumber(P+1)+
                           '】区间局部矿压异常区域信息表',4);
                 Add_Third_three_Main_Chlid(p,P_c,-1,P_Str,inGZm);
             end;
             ...
             if ComeStep>0  then   begin
                 ...
                 if ComeStep > 0 then  begin
                     ...
                     SetHeading('表'+P_Str+'-'+intToStr(Third_Tab_id)+' 第【'+Public_Basic.GetChineseNumber(P+1)+
                           '】区间未采区域矿压显现显著预测信息表',4);
                     AddReturn();
                     Add_Third_three_Main_Chlid(p,P_c,30,P_Str,inGZm);
                 end;
                 ...
                 if ComeStep > 0 then  begin
                     ...
                     SetHeading('表'+P_Str+'-'+intToStr(Third_Tab_id)+' 第【'+Public_Basic.GetChineseNumber(P+1)+
                           '】区间未采区域矿压显现明显预测信息表',4);
                     AddReturn();
                     Add_Third_three_Main_Chlid(p,P_c,27,P_Str,inGZm);
                 end;
             end;

            SetFont('宋体',12,false,false);AlignLine(0);
            WriteText( Add_Third_three_Main_Chlid_Add_fulu_Bmp(p) ,1);
            AddReturn();
         end;
    end;
end;

procedure TWordReport.Add_Third_three_Main_Chlid(phaseNumber,P_c,modify: integer;
       ...
       for I := 0 to high(iDWGrap.Auto_Judge_OldStep.PhaseSteps[phaseNumber].Steps) do  begin
              ...
              if not((iDwGrap.Auto_Judge_OldStep.PhaseSteps[phaseNumber].Steps[i].EndDao>iDwGrap.Auto_Judge_OldStep.PhaseSteps[phaseNumber].Steps[i].StartDao ) and
                 (iDwGrap.Auto_Judge_OldStep.PhaseSteps[phaseNumber].Steps[i].EndSup>iDwGrap.Auto_Judge_OldStep.PhaseSteps[phaseNumber].Steps[i].StartSup )) then
                     continue;
             // 把数据写入表格
             Add_Third_Two_First_Table_Child(phaseNumber+1,iDwGrap.Auto_Judge_OldStep.PhaseSteps[phaseNumber].Steps[i],
...
function TWordReport.Add_Third_three_Main_Chlid_Add_fulu_Bmp(
...
procedure TWordReport.Add_Third_Two_First_Table_Child(phaseNumber:integer;OneStepIn: TcomprehensiveOldStep;
...
begin
     ...
     innWord.OpenWord('');
     //
     iCallPStope.SetFyyd_Bmp(AHandle);
     iCallPStope.GetDTFZBmp(AHandle);
