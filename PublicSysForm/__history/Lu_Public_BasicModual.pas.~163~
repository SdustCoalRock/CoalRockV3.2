...
interface
 ...
 TLu_BasicClass=Class
   private
      In_UseUnitName:string;  {使用单位情况}
      in_Licensing:Boolean;  {许可是否通过}
   ...
   public
      ...
      procedure WriteIniTimeInterval(Vaule:Integer); //写ini计算时间间隔
      ...
      procedure WriteDisplayWholeScreen(Value:integer);//写是否全屏显示
      function ReadDisplayWholeScreen:integer;//读是否全屏显示
      procedure WriteSnapData(Value:Boolean;ip,Port:string);//写是否使用第三层
      function  ReadSnapData(var snapIP,SnapPort:String):Boolean;// 读是否使用第三层数据
      procedure WriteUpdateCoalBmp(Value:Boolean); // 写更新本地矿图文件
      function  ReadUpdataCoalBmp:Boolean; // 读更新本地矿图文件
      procedure WriteMakeWordPercent(state:Boolean;Percent:integer); // 把 打印报告的 进度写进去
      function  ReadMakeWordPercent(Var Percent:integer):Boolean; // 把打印报告的进度读出来
     //
      {读写数据类型编号}
...
begin
    ...
    Imag.Picture.Bitmap.Canvas.Font.Name :='宋体';
...
begin
   ...
   StatUsBarRightStr:='宋振骐院士团队研发 CopyRight (2016-2018)';
    //日期格式转换
  ...
  try
    ...
    // 合并其他单元格
     for I := 0 to High(arry) do  begin
        ...
        xlsheet.cells[arry[i].FromRow+2,arry[i].FromCol+1].value := Meger_Str; //页头第一行；
     end;
    //
...
begin
   ...
   if  not assigned(fReadini) then
     ...
     fReadini.iniSetStr('郑煤机使用', 'First',SetString);
     fReadini.iniSetStr('覆岩运动规律', 'Second',SetString);
     fReadini.iniSetStr('矿压动态分析', 'Third',SetString);
     fReadini.iniSetStr('宋院士模拟平台', 'Fouth',SetString);
     fReadini.iniSetStr('神东三维曲面矿压分析', 'FiFth',SetString);
end;
