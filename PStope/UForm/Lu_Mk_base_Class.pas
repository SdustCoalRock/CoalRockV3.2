 ...
 //定义岩层类 TYc_Rcok
  ...
  TOld_roof=class
      public
        ...
        YL_First_step     : array[0..2] of double;    //综合岩梁初次来压步距  0 为最大值，1为最小值，2为平均值
        YL_Zhouqi_step    : array[0..2] of double;    //综合岩梁周期来压步距  0 为最大值，1为最小值，2为平均值
        YL_YL_first_step     : array[0..5,0..2] of double;    //各岩梁综合初次来压步距  0 为最大值，1为最小值，2为平均值
        YL_YL_Zhouqi_step    : array[0..5,0..2] of double; //岩梁综合周期来压步距  0 为最大值，1为最小值，2为平均值
        YL_YL_M              : array[0..5,0..2] of double; //岩梁综合厚度  0 为最大值，1为最小值，2为平均值
        YL_Step_struct: integer; // 步距法 岩梁结构
        YL_Dep_struct:integer; // 裂断法 岩梁结构
        is_init      :bool;
  ...
  TDraw_init= Class
     ...
     //  i,j,k     :integer;   //  临时变量
       ...
       old:Told_roof;
       // procedure
       procedure initImage(); //初始化image变量    1
       procedure Maketitle(DefOrStep:string); //定制标题   2
       procedure BeamStruct();   //3
       procedure CountZK_H();  // 4 计算岩梁高度
       procedure ImageBlank(); //5
           ...
           begin
             if length(temp2[i])=0 then continue;
                  case i of
                     ...
                     {self.S_cmfs 1 炮采，2 普采，3 综采，4，放顶煤）}
                    ...
                    if strtoint_lu(yc4[9])=0 then
                     ...
                     //------数据判断
                       if Yc_Rock[i-1].R_h =0 then    // 判断岩层的厚度
                             begin
                                Result:=False;
                                sEorr:=sEorr+'/岩层'+yc4[0]+'的厚度有误!';
                             end;

                      if Yc_Rock[i-1].R_klqd  =0 then    // 判断岩层 抗拉强度
                             begin
                               Result:=False;
                               sEorr:=sEorr+'/岩层'+yc4[0]+'的抗拉强度有误!';
                             end;
                       if Yc_Rock[i-1].R_kyqd =0 then    // 判断岩层 抗压强度
                             begin
                               Result:=False;
                               sEorr:=sEorr+'/岩层'+yc4[0]+'的抗压强度有误!';
                             end;
                      if Yc_Rock[i-1].R_r  =0 then    // 判断岩层 容重
                             begin
                               Result:=False;
                               sEorr:=sEorr+'/岩层'+yc4[0]+'的容重有误!';
                            end;
                       if mc_count>1 then
                             begin
                               Result:=False;
                               sEorr:=sEorr+' /传入了多层主采煤层标记!';
                             end;

                       //计算岩层的步距
                       if Yc_Rock[i-1].R_sffc then begin
                           self.Yc_Rock[i-1].Step_Rock(Yc_Rock[i-1].R_fchd ,Yc_Rock[i-1].R_klqd ,Yc_Rock[i-1].R_r );
                       end else begin
       ...
       try
         ...
         //采高大于钻孔岩层高度的处理   2016.505
          if s_zk.Yc_Rock[s_zk.main_coal].R_h< s_gzm.S_Cg_h then
             s_gzm.S_Cg_h:=s_zk.Yc_Rock[s_zk.main_coal].R_h  ;
        //--------------计算放顶煤的参数
   ...
   try
     ...
     for i:= 0 to 4 do   begin
         if (i+1 > YL_Step_struct) and (i+1 > YL_Dep_struct) then  begin
            ...
            YL_YL_First_step [i][0]:=select_max(s_zk.Beam_Def[i].Step_0_Compr,
                              s_zk.Beam_Def[i].Step_0_Simple ,
                              s_zk.Beam_step[i].Step_0_Compr ,
                              s_zk.Beam_step[i].Step_0_Simple);
            YL_YL_First_step [i][1]:=select_min(s_zk.Beam_Def[i].Step_0_Compr,
                              s_zk.Beam_Def[i].Step_0_Simple ,
                              s_zk.Beam_step[i].Step_0_Compr ,
                              s_zk.Beam_step[i].Step_0_Simple);
...
function Told_roof.Return_C0(zk:Tzk_bore):wideString;  stdcall;
 ...
 function Cal_Zcyl(zk:Tzk_bore;imm:TImmediate_roof;gzm:TG_stope;old_r :Told_roof;out S_ZCYl:WideString):wordbool;
  ...
  begin
     ...
     //判断赋值  20140103
     //通过煤层厚度修订参数
      if  zcyl_s1<2 then   begin
        if gzm.S_Cg_h>3 then begin
           gzm.ZCYL_s1[2]:=zcyl_s1*3;    //通过煤层厚度修订参数
        end else  if gzm.S_Cg_h>1 then begin
           gzm.ZCYL_s1[2]:=zcyl_s1*gzm.S_Cg_h;
        end else begin
           gzm.ZCYL_s1[2]:=zcyl_s1;
        end;

      end else if zcyl_s1<5 then begin
        if gzm.S_Cg_h>3 then begin
           gzm.ZCYL_s1[2]:=6+(zcyl_s1-2)*1.5;    //通过煤层厚度修订参数
        end else  if gzm.S_Cg_h>1 then begin
           gzm.ZCYL_s1[2]:=2*gzm.S_Cg_h+(zcyl_s1-2)*gzm.S_Cg_h*0.5;
        end else begin
           gzm.ZCYL_s1[2]:=zcyl_s1;
        end;
      end else begin
...
//  拆分传入支架结构参数
 ...
 //计算支架基本参数
procedure TZJ_Class.Calculate_weight(gzm:TG_Stope;Zk:Tzk_bore;old:Told_roof;defOrStep:string;BeamNumber:integer;
                    var maxV,minV :double);
begin
   maxV:=0;MinV:=0;
...
procedure TZJ_Class.Cal_empirical_formula3(gzm: TG_Stope;imm:TImmediate_roof;old:Told_roof);
var
   N: double;
begin
    { 史学伟经验公式
...
procedure TZJ_Class.Having_stress(gzm: TG_Stope; Zk: Tzk_bore; mm: TImmediate_roof; old: Told_roof;
          idefOrStep:string;BeamNumber:integer;MaxV,MinV:double);
var
  ...
  //取得Dll自身路径
     ...
     // ShowMessage(dll_Path);
      ...
      if NQ_step='1' then begin   //1 挠曲 算法
            TitleText:='【'+Gzm.S_Name+'】工作面_【'+zk.Zk_name+'】钻孔数据(挠曲算法):支架岩梁关系图';
            for I := 0 to 4 do
               Old_beam[i]:=zk.Beam_Def[i] ;
      end else begin          //步距算法
            TitleText:='【'+Gzm.S_Name+'】工作面_【'+zk.Zk_name+'】钻孔数据(步距算法):支架岩梁关系图';
             for I := 0 to 4 do
...
procedure TDraw_init.BeamStruct;
var
  k:integer;
begin
    //判断岩梁的高度
...
procedure TDraw_init.CountZK_H;
var
  i:integer;
begin
     Y_tu_H:=0;
...
procedure TDraw_init.ImageBlank;
begin
     Y_Text_Bl:=(Image_out.Height-TopBlank-BottomBlank-X_Text_height-40-Table_Title_H-(Yl_name_count)*HanziHeight)/Y_tu_H;
     Y_tu_BL:=(Image_out.Height-TopBlank-BottomBlank-X_Text_height-40)/Y_tu_H;
...
procedure TDraw_init.initImage;
begin
    //取得Dll自身路径
      dll_Path:=zhijia.OcxPath ;
     // ShowMessage(dll_Path);
...
procedure TDraw_init.Maketitle(DefOrStep:string);
var
  i:integer;
begin
        //定标题
