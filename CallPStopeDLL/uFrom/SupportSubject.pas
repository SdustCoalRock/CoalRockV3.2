...
begin
        //树图初始化
    MySqlDataSet1.Close;
    sql:=  'SELECT SD.id as id,SD.SubjectText as SubText  from supSubjectData as SD where id in (select SubJ_id ' +
                   ' from  SupportSubIndex As SI where    SI.Firstid='+IntTostr(First)+' and secondid ='+IntToStr(secondid) +
                   ' )' ;
...
procedure TForm_Subject.DeleteButtonClick(Sender: TObject);
var
  t_s:Widestring;
begin
   if  Subjectid<1  then exit;
    t_s:='你确定要删除选中编号为【'+IntTostr(Subjectid)+'】的数据么?';
    if Application.MessageBox(Pwidechar(t_s),'提示',MB_OKCANCEL+MB_ICONQUESTION) =2 then  exit;
  try
        MyCommand1.CommandText:='delete from supsubjectData where id='+IntTostr(Subjectid);
        MyCommand1.Execute(MainDataModule.ExUserName) ;
        MyCommand1.CommandText:='delete from supportsubindex where subj_id='+IntTostr(Subjectid);
        MyCommand1.Execute(MainDataModule.ExUserName) ;
...
begin
  ...
  try
    ...
    MySqlDataSet1.Close ;
    sql:='SELECT SS.firstid as First_id,SS.secondid as Second_ID from SupportSubIndex as SS ' +
         ' where SS.SubJ_id =' +IntToStr(Subid);
    MySqlDataSet1.CommandText:= sql;
...
begin
    if  SaveSubjectIntoData then begin
...
begin
 ...
 try
      ...
      sql:=' insert into SupportSubIndex (SubJ_id,firstid,secondid) values ( ' +
            intTostr(Subid) +',';
      for I := 0 to t_SupSubject.Child_count-1 do
         for j := 0 to t_SupSubject.Child_Second[i].Child_count-1 do
