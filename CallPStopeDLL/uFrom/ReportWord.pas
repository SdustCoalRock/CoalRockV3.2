...
begin
     with innWord do begin
          ...
          AlignLine(1);
          SetHeading('二、钻孔信息',2);// 章节
          AddReturn();
           //正文
          SetFont('宋体',13,false,false);
          AlignLine(0);
          WriteText('    该次测算使用的钻孔名称为：'+inGzm.UsedDrill.Zk_name +
                    '，该钻孔录入岩层：'+IntToStr(inGzm.UsedDrill.Zk_Yc_Count)+'层。'+#13,0);

          AlignLine(1);
          SetFont('宋体',10,true,false);
          AddTable(6,inGzm.UsedDrill.Zk_Yc_Count);
          WriteText('岩层序号',0);   MoveRight(1);
          WriteText('岩层名称',0);   MoveRight(1);
          WriteText('均厚(米)',0);   MoveRight(1);
          WriteText('岩石容重',0);   MoveRight(1);
          WriteText('抗压MPa',0);    MoveRight(1);
          WriteText('抗拉MPa',0);    MoveRight(1);
    ...
    with innWord do begin
         ...
         try
             ...
             if MainDataModule.MyDataSet.RecordCount >0 then begin
                WriteText('存在类似的液压支架：',0); AddReturn();
                ...
                WriteText('数据库中不存在相似的液压支架：',0); AddReturn();
             end;
         finally
            MainDataModule.MyDataSet.Close ;
         end;
...
begin
    ...
    with innWord do begin
          ...
          AddReturn();
          SetFont('宋体',13,false,false);
          WriteText('   直接顶运动参数：'+StrPas(Main_CallPStope.UsePStope.Get_immediate_roof('1.0')),0);
          AddReturn();
          WriteText('   老顶运动参数：'+StrPas(Main_CallPStope.UsePStope.Get_Old_roof('1.0')),0);
          AddReturn();
          {为沿空留巷定制}
          if ReportType='Yklh' then begin
              SetFont('黑体',13,false,false);
              SetHeading('3、承压力分布规律',3);
              AddReturn();
              SetFont('宋体',13,false,false);
              Report_Zcyl(inGzm);
          end;
...
begin
   ...
   try
    with innWord do begin
          ...
          AddReturn();   //加一个空行
          SetFont('宋体',15,false,false);
          SetHeading('五、沿空留巷可行性分析方案',2);// 章节
          AddReturn();
           //正文
          SetFont('黑体',13,false,false);
          SetHeading('1、工作面顶板分布信息',2);
          AddReturn();
          AlignLine(1);
          SetFont('宋体',10,true,false);
          AddTable(8,2);
...
//*************
     ...
     if C1> 4 then  begin
         ...
         with innWord do begin
             WriteText('    内应力场峰值'+s_temp[0],0);
                AddReturn();
             WriteText('    内应力场'+s_temp[1]+'米',0);
                 AddReturn();
             WriteText('    外应力场峰值'+s_temp[2],0);
                 AddReturn();
             WriteText('    支承压力峰值距离煤壁距离'+s_temp[3]+'米',0);
                  AddReturn();
             WriteText('    外应力范围'+s_temp[4]+'米',0) ;
                 AddReturn();
         end;
